...
public class TileEntityTemperer extends TileEntity implements IInventory, ITickable
{
	...
	public static boolean isBurning(TileEntityTemperer te) 
	{
	...
	{	
		...
		{
			--this.burnTime;
			BlockTemperer.setState(true, world, pos);
		}
		...
		else
		{
			...
			{
				ItemStack output = TestererRecipes.getInstance().getTestererResult(inputs[0], inputs[1]);
				if(!output.isEmpty())
				{
					smelting = output;
					cookTime++;
	...
	private boolean canSmelt() 
	{
		...
		{
			ItemStack result = TestererRecipes.getInstance().getTestererResult((ItemStack)this.handler.getStackInSlot(0), (ItemStack)this.handler.getStackInSlot(1));	
